# export to eps file 
setEPS()
postscript("result_bar_plot_rmse.eps",height=6.85, width=12)
...
dataRawRMSE <- read.csv("../data/rmse_all_models.csv", sep=',',header = T, encoding='utf-8')
...
dataMean <-data.frame(dataRawRMSE$GEDE,dataRawRMSE$KIDW.TCGRU,dataRawRMSE$BiLSTM.IDW)
max_y = max(dataRawRMSE$GEDE,dataRawRMSE$KIDW.TCGRU,dataRawRMSE$BiLSTM.IDW)
...
# draw the bars
barCenters <- barplot(dataMean, beside = TRUE, ylim=c(0,10), names.arg =c("testcase 1", "testcase 2","testcase 3", "average"),
                      angle=c(100,35,120),
                      density=c(6,12,20),
                      #xlab = "Test number",
                      ylab = "RMSE",
...
bar2<- barplot(dataMean,add=TRUE, ylim=c(0,10),cex.lab = 2, cex.axis=2, cex.names=2,beside = TRUE,col=plot_colors,
               angle=c(100,35,120),
               density=c(6,12,20))
