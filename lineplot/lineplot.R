##### read data 
data<-read.csv("../data/Station_24_cut_bilstm.csv",header=T,sep=",")
data
##### define data range 


max_x <- max(data$X) ## minimum x 
min_x <- min(data$X)  ## maximum x
max_y <- max(data$Groundtruth, data$Predict)  ## minimum y
min_y <- min(data$Groundtruth, data$Predict)  ## maximum x
### max_e <- max(data$inma_e, data$qlearning_e, data$nocharge_e)  ### maximum error 

##### export eps file 
setEPS()
postscript("Station24_lineplot_IDW_BILSTM.eps",height=6.5, width=6.5)
...
##### cex is the size 
plot(c(min_x,max_x),c(min_y, max_y+10), type="n", xaxt="n", cex.lab = 1.5, cex.axis=1.5, xlab="Date", ylab=expression(paste("PM2.5 (", mu, "g", "/", m^3,")")))
axis(1, at = seq(0, 200, by = 20), las=1,  cex.axis=1.5)
...
lines(data$X, data$Groundtruth, type="l", pch=0, lty=1, col=plot_colors[1], lwd=2.5, cex = 1.6)
lines(data$X,data$Predict, type="l", pch=1, lty=1, col=plot_colors[2], lwd=2.5, cex = 1.6)
...
###### to draw the legend
legend(bty = "n","topright", c("Air Quality", "IDW-BiLSTM"),pch=c(0,1,2,5), lty=c(1,1,1,1), col=plot_colors, lwd=1, cex=1.6, x.intersp=1, y.intersp=1.5)
